...
tags:
  - 通识
excerpt: 通识领域索引：涵盖开发环境配置、版本管理、包管理器换源、虚拟机与 ESXI 运维、周边硬件及实用工具的使用记录。
---
...
- [通识](#通识)
  ...
  - [硬件](#硬件)
    - [鼠标](#鼠标)
      - [罗技 Master2S 按键正常, 光标无法移动](#罗技-master2s-按键正常-光标无法移动)
  - [杂项](#杂项)
    - [Prettier](#prettier)
      - [收起自动格式化展开的列表](#收起自动格式化展开的列表)
...
![image-20251017231416888](http://cdn.ayusummer233.top/DailyNotes/202510172314960.png)

当上图中 `multilineArraysWrapThreshold` 设置为小于 4 的值时, 左边的 roi 列表会被自动格式化展开成四行, 但是这明显不符合我们实际的需求, 因此我们可以将其改为大于 4 的数字

不过相应的带来一个衍生问题, 那就是修改后的 prettier 配置并不能帮助我们把先前自动展开成四行的 ROI 列表缩回去, 因为因为 `prettier-plugin-multiline-arrays` 会优先保留“手动换行”的数组：只要原文件里 `[` 后已经换过行，它就认为这是你刻意分行的结果，即便阈值放宽也不会自动收回单行。
要想利用 prettier 便捷地将展开的列表收回去, 可以在 json 的首行加上如下注释强制插件忽略已有的手动换行

```json
// prettier-multiline-arrays-set-threshold: 99
```

![image-20251017231810138](http://cdn.ayusummer233.top/DailyNotes/202510172318206.png)

然后 `Shift+Alt+F` 或者 npx prettier --write xxx/xxx.json 即可自动收回展开的列表, 收回后将注释再删除即可
